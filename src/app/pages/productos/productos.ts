...
import {
  ListaProductosModel,
  ListatipoProductosModel,
  ProductosModel,
} from '../../model/productosModel';
...
export class Productos {
  ...
  deleteProduct(product: ListaProductosModel) {
    this.confirmationService.confirm({
      message:
        'Está seguro de desactivar el producto: ' + product.descripcion + '?',
      ...
      accept: () => {
        this.listaProductos.set(
          this.listaProductos().filter(
            (val) => val.id_productos !== product.id_productos
          )
        );
  ...
  saveProducto() {
    ...
    this.productosService
      .registrarProductos(this.subirProducto(), this.op)
      .subscribe({
        next: (data) => {
          this.cargarlista();
          this.spinner = false;
